...
public class GameActivity extends Activity implements OnSharedPreferenceChangeListener {
	...
	private static List<Pile> pileLayouts;
	private static List<FoundationPile> foundationPiles;
	private static List<CellPile> cellPiles;
	...
	private void restoreGame() {
		...
		for (int k = 0; k < Game.START_CARDS_DEAL_COUNT; k++) {
			Pile pile = pileLayouts.get(k % pileLayouts.size());

			Card card = deck.getCard();
			Game.addCardDealt();
			// boolean faceUp = Game.START_CARDS_DEAL_COUNT - k <=
			// pileLayouts.size();
			card.setFaceup(true);
			pile.addCard(card);
	...
	private void findViewsSetListenersAndManagers() {
		...
		cellPiles = new LinkedList<CellPile>();
		cellPiles.add((CellPile) findViewById(R.id.slotPile0));
		cellPiles.add((CellPile) findViewById(R.id.slotPile1));
		cellPiles.add((CellPile) findViewById(R.id.slotPile2));
		cellPiles.add((CellPile) findViewById(R.id.slotPile3));

		i=100;
		for(CellPile pile: cellPiles) {
			pile.setTag(i++);
		}

		foundationPiles = new LinkedList<FoundationPile>();
		foundationPiles.add((FoundationPile) findViewById(R.id.winningPile0));
		foundationPiles.add((FoundationPile) findViewById(R.id.winningPile1));
		foundationPiles.add((FoundationPile) findViewById(R.id.winningPile2));
		foundationPiles.add((FoundationPile) findViewById(R.id.winningPile3));
	...
	private void dealCard(Pile pile, Card card, boolean faceup, int hundredMillisecondOffset) {
		...
		float stackMargin = 0;
		//		if (isNewGameDeal) {
		stackMargin = getResources().getDimension(R.dimen.card_stack_margin_up) * cardsOffset;
		//
		//		} else {
	...
	private void setupNewGame() {
		...
		for (Pile pile : pileLayouts) {
			pile.removeViews(1, pile.getCardsCount());
		}
		for (CellPile pile : cellPiles) {
			pile.removeViews(1, pile.getCardsCount());
		}
	...
	private void undo(boolean human) {
		...
		// if (move.getAction() == Move.ACTION_DEAL && human &&
		// !chosenUnrestrictedUndo) {
